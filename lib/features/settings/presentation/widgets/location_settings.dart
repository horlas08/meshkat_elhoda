...
class _LocationSettingsState extends State<LocationSettings> {
  ...
  Future<void> _manualRefresh() async {
    ...
    _requestLocationRefresh(const RefreshLocationIfNeeded(forceRefresh: true));
  }

  void _requestLocationRefresh(LocationEvent event) {
    ...
    context.read<LocationBloc>().add(event);
    _startRefreshTimeout();
  }

  void _startRefreshTimeout() {
    _refreshTimeoutTimer?.cancel();
    _refreshTimeoutTimer = Timer(const Duration(seconds: 15), () {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            isArabic
                ? 'انتهت مهلة تحديث الموقع. حاول مرة أخرى.'
  ...
  void _finishRefresh({required bool success, String? message}) {
    if (!_isRefreshing) return;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          success ? s.locationUpdatedSuccessfully : message ?? s.unknown,
          style: TextStyle(fontFamily: AppFonts.tajawal),
        ),
        backgroundColor: success ? AppColors.goldenColor : Colors.red.shade500,
      ),
    );
  }
        ...
        actions: [
          ElevatedButton(
            onPressed: () {
              ...
              // طلب إذن الموقع
              _requestLocationRefresh(RequestLocationPermissionEvent());
            },
    ...
    return BlocListener<LocationBloc, LocationState>(
      listener: (context, state) {
        if (state is LocationGranted) {
          _finishRefresh(success: true);
        } else if (state is LocationDenied) {
          _finishRefresh(success: false, message: state.message);
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: SwitchListTile(
                    ...
                    activeThumbColor: AppColors.goldenColor,
