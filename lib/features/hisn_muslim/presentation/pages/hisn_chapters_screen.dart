...
class HisnChaptersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      create: (context) => getIt<HisnMuslimBloc>()..add(LoadHisnChapters(languageCode)),
      child: Scaffold(
        appBar: AppBar(
          ...
          builder: (context, state) {
            if (state is HisnMuslimLoading) {
              ...
              return RefreshIndicator(
                onRefresh: () async {
                   // Since key handles the reload, this might just need to re-add event to current context
                   context.read<HisnMuslimBloc>().add(LoadHisnChapters(languageCode));
                },
